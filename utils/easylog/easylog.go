...
// Log 在性能良好但不重要的情况下，请使用 SugaredLogger。 它比其他结构化日志记录包快 4-10 倍，并且包括结构化和 printf 样式的 API。
...
func Init() {
	conf := LogConfigs{
		LogLevel:          "info",      // 输出日志级别 "debug" "info" "warn" "error"
		LogFormat:         "logfmt",    // 输出日志格式 logfmt, json
		LogPath:           "/log",      // 输出日志文件位置
		LogFileName:       "wbapi.log", // 输出日志文件名称
		LogFileMaxSize:    2,           // 输出单个日志文件大小，单位MB
		LogFileMaxBackups: 100,         // 输出最大日志备份个数
...
func getEncoder(conf LogConfigs) zapcore.Encoder {
	...
	encoderConfig.EncodeTime = conf.EncodeTime              // log 时间格式 例如: 2021-09-11t20:05:54.852+0800
